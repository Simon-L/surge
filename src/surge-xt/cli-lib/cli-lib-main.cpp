...
int main(int argc, char **argv)
{
    ...
    std::string initPatch = "/home/xox/Sync/SyncMore/surge/resources/data/patches_factory/Templates/Init Saw.fxp";
    ...
    int oscInputPort{9000};
    int oscOutputPort{9001};
    std::string oscOutputIPAddr{"127.0.0.1"};
    ...
    float* inputChannelData[2];
    inputChannelData[0] = new float[512];
    inputChannelData[1] = new float[512];
    int numInputChannels{2};
    float* outputChannelData[2];
    outputChannelData[0] = new float[512];
    outputChannelData[1] = new float[512];
    int numOutputChannels{2};
    int numSamples{512};
    
    juce::MidiKeyboardState source;
    engine->proc->surge->playNote(0, 60, 127, 0, -1);
    ...
    while (continueLoop)
    {
        if (needsMessageLoop)
        {
            ...
            engine->audioDeviceIOCallbackWithContext(inputChannelData, numInputChannels, outputChannelData, numOutputChannels, numSamples, ctx);
            for (size_t s = 0; s < numSamples; s++) {
                std::cout << s << " -> " << outputChannelData[0][s] << '\n';
            }
            // mm->runDispatchLoop();
        }
        else
